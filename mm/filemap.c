...
no_page:
	...
	if(current->pid >2000)
		if(task_css_set(current)->subsys[3]!=NULL)
			if(page&&(page->weight>=100)&&(page->weight<=task_css_set(current)->subsys[3]->cgroup->weight)&&task_css_set(current)->subsys[3]->cgroup->weight!=500){
//		if(page->weight!=500){
//		if(strcmp(current->comm,"fio")==0||strcmp(current->comm,"iozone")==0||strcmp(current->comm,"filebench")==0){
		
	//		printk("%s page->weight:%d to %d\n",current->comm,page->weight,task_css_set(current)->subsys[3]->cgroup->weight);
			//page->weight_ref=(task_css_set(current)->subsys[3]->cgroup->weight)/100-2-(page->weight/100-page->weight_ref);
					page->weight_ref = task_css_set(current)->subsys[3]->cgroup->weight/100-1; 
